...
def test_username_can_contain_letters_digits_and_underscores():
    assert validate_email("a_a1a-a@nomewebsite.ext") is None


def test_username_should_starts_with_letters():
    assert validate_email("a@nomewebsite.ext") is None


def test_username_when_doesnt_strat_with_letters_raise_exception():
...
def test_extension_should_contain_only_three_chars():
    assert validate_email("abc@wensite123.com") is None
